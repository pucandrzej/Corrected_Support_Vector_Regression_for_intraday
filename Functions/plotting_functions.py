...
def plot_avg_day_trade(
    ...
    dates,
    title="",
    week_days_chosen=range(7),
    ax=None,
    cbar_label="",
    save_title="3D_analysis",
    xlabel=False,
    ylabel=False,
    ...
    for wd in week_days_chosen:
            ...
            for idx0, dminute in enumerate(dates_wday):
                if dday == dminute.normalize():
                    ...
                    daily_counter_1[:, idx] = np.nansum(
                        np.dstack((counter_wday[:, idx0] > 0, daily_counter_1[:, idx])),
                        2,
                    )
        ...
        if ax == None:
            ...
            plt.savefig(f"Paper_Figures/{save_title}-{weekd[wd]}.pdf")
            plt.show()
        else:
            if cbar_limits != None:
                im = ax.imshow(
...
def plot_avg_day_trade_glued(
    ...
    for wd in week_days_chosen:
            ...
            for idx0, dminute in enumerate(dates_wday):
                if dday == dminute.normalize():
                    ...
                    daily_counter_2[:, idx] = np.nansum(
                        np.dstack((counter_wday[:, idx0] > 0, daily_counter_2[:, idx])),
                        2,
                    )
        ...
        if return_min_max:
            return (
                np.nanmin([np.nanmin(daily_array_1), np.nanmin(daily_array_2)]),
                np.nanmax([np.nanmax(daily_array_1), np.nanmax(daily_array_2)]),
            )
...
def plot_avg_day_trade_glued_counter(
    df_copy,
    prices_1,
    counter_1,
    prices_2,
