...
required_start = datetime(2018,10,31) # we cannot go back due to the lack of Load forecasts in few days there
...
def fill_forecasts(load_df, actual="Actual", forecast="Forecast",
                   lookback_days=30, seed=None):
    ...
    for t in df.index[df[forecast].isna() & df[actual].notna()]:
        ...
        df.at[t, forecast] = df.at[t, actual] - sampled_err # put the sampled value in place of NaN forecast
...
for year in range(2018, 2021):
    dayahead.append(pd.read_csv(os.path.join("Day-Ahead-Quarterly-Data", f"Day-ahead Prices_{year}.csv"), na_values=['n/e']))
...
dayahead_df['Time from'] = pd.to_datetime(new_datetimes, format="%d.%m.%Y %H:%M", dayfirst=False, yearfirst=False)
...
total_df = pd.concat(
    [
        pd.read_csv("ID_auction_preprocessed/intraday_auction_spot_prices_15-call-DE_2018.csv", skiprows=1, index_col=0, parse_dates=True, dayfirst=True),
        pd.read_csv("ID_auction_preprocessed/intraday_auction_spot_prices_15-call-DE_2019.csv", skiprows=1, index_col=0, parse_dates=True, dayfirst=True),
        pd.read_csv("ID_auction_preprocessed/intraday_auction_spot_prices_15-call-DE_2020.csv", skiprows=1, index_col=0, parse_dates=True, dayfirst=True)
]
).iloc[:, :-8]
...
ge_fr = []
for year in range(2018, 2021):
    ge_fr.append(pd.read_csv(f"Crossborder/crossborder_ge_fr_{year}.csv", na_values=['n/e']))
...
load = []
for year in range(2018, 2021):
    load.append(pd.read_csv(f"Load/Total Load - Day Ahead _ Actual_{year}.csv", na_values=['n/e']))
...
gen_fore = []
for year in range(2018, 2021):
    gen_fore.append(pd.read_csv(f"Generation/generation_fore_{year}.csv", na_values=['n/e']))
