...
def plot_cv_results(parameters, input_mae, model, sample_length, params_index):
    for kernel_model in parameters.keys():
        ...
        fig.add_trace(go.Scatter(
            ...
            mode='lines+markers',
            name='Mean MAE',
            text=[str(params) for params in parameters[kernel_model]],  # Add parameter info as hover text
            hoverinfo='text+y'
        ))
        ...
        # Show the plot
        fig.write_html(os.path.join('CV_RESULTS', 'PLOTS', f'{model}_{sample_length}_{params_index}_{kernel_model}_2D_plot.html'))

        # 3D plots
        # Extract unique C and epsilon pairs
        if kernel_model == 'SVR':
            unique_pairs = list({(p['C'], p['epsilon']) for p in parameters[kernel_model]})

            # Create 3D plots for each unique (C, epsilon) pair
            for C, epsilon in unique_pairs:
                # Filter data for the current pair
                filtered_data = [p for p in parameters[kernel_model] if p['C'] == C and p['epsilon'] == epsilon]
                indices = [i for i, p in enumerate(parameters[kernel_model]) if p['C'] == C and p['epsilon'] == epsilon]
                ...
                fig.write_html(os.path.join('CV_RESULTS', 'PLOTS', f'{model}_{sample_length}_{params_index}_{kernel_model}_{round(C, 3)}_{round(epsilon, 3)}_3D_plot.html'))

        elif kernel_model == 'cSVR':
            # Extract values from parameter_data
            q_data_naive = np.array([p['q_data_naive'] for p in parameters[kernel_model]])
            q_kernel_naive = np.array([p['q_kernel_naive'] for p in parameters[kernel_model]])
            ...
            # Show the figure
            fig.write_html(os.path.join('CV_RESULTS', 'PLOTS', f'{model}_{sample_length}_{params_index}_{kernel_model}_3D_plot.html'))

        elif kernel_model == 'ccSVR':
            # Extract unique div_impact values
            unique_div_impact = np.unique([p['div_impact'] for p in parameters[kernel_model]])
            ...
            for div_impact in unique_div_impact:
                ...
                filtered_data = [p for p in parameters[kernel_model] if p['div_impact'] == div_impact]
                indices = [i for i, p in enumerate(parameters[kernel_model]) if p['div_impact'] == div_impact]
                ...
                # Populate mae_grid with corresponding MAE values
                for i, (qk, qd, mae) in enumerate(zip(q_kernel_div, q_data_div, mae_values)):
                    grid_idx = np.where((q_kernel_grid == qk) & (q_data_grid == qd))
                    if grid_idx[0].size > 0 and grid_idx[1].size > 0:
                        mae_grid[grid_idx] = mae
                ...
                fig.write_html(os.path.join('CV_RESULTS', 'PLOTS', f"{model}_{sample_length}_{params_index}_{kernel_model}_3D_plot_div_impact_{round(div_impact, 3)}.html"))
