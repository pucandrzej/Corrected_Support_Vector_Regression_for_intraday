...
def kernel_target_alignment(K, y):
    return(
            (
                np.expand_dims(y, axis=(1)).T @ K @ np.expand_dims(y, axis=(1))
            )[0][0]/(
...
def predict(model, X_train, Y_train, X_test, q_kernel=None, q_data=None, naive_forecasts=None, naive_forecast=None, q_kernel_naive=None, q_data_naive=None, q_kernel_div=None, q_data_div=None, div_impact=None, epsilon=None, C=None, insample_mae=True):
    ...
    if model in ['AR', 'AR_exp', 'ARX', 'ARX_exp', 'AR_noshock', 'AR_exp_noshock', 'ARX_noshock', 'ARX_exp_noshock']:
        ...
        plain_kernel_X = calc_interm_kernel(X_train)
        plain_kernel_X_naive = np.abs(np.tile(naive_forecasts, [len(naive_forecasts), 1]) - np.swapaxes(np.tile(naive_forecasts, [len(naive_forecasts), 1]), 0, 1))**2
        sigma = np.quantile(plain_kernel_X_naive, q_data_naive) / scipy.stats.norm.ppf(q_kernel_naive, loc=0, scale=1)
        width = laplace_quantile/np.quantile(plain_kernel_X, q_data)
        training_matrix = np.exp(-np.abs(width) * plain_kernel_X - 1/(2*sigma**2) * plain_kernel_X_naive)
        reg = SVR(kernel = 'precomputed', epsilon = epsilon , C = C)
        kernel_alignment = kernel_target_alignment(training_matrix, Y_train)
        reg.fit(training_matrix, Y_train)
        ...
        sigma = np.quantile(plain_kernel_X_naive, q_data_naive) / scipy.stats.norm.ppf(q_kernel_naive, loc=0, scale=1)
        width = laplace_quantile/np.quantile(plain_kernel_X, q_data)
        test_matrix = np.exp(-np.abs(width) * plain_kernel_X - 1/(2*sigma**2) * plain_kernel_X_naive)
        ...
        plain_kernel_X_naive = np.abs(np.tile(naive_forecasts, [len(naive_forecasts), 1]) - np.swapaxes(np.tile(naive_forecasts, [len(naive_forecasts), 1]), 0, 1))**2
        sigma = np.quantile(plain_kernel_X_naive, q_data_naive) / scipy.stats.norm.ppf(q_kernel_naive, loc=0, scale=1)
        plain_kernel_X_naive_div = np.abs(np.tile(naive_forecasts, [len(naive_forecasts), 1]) - np.swapaxes(np.tile(naive_forecasts, [len(naive_forecasts), 1]), 0, 1))
        plain_kernel_X_naive_div_fore = np.abs(naive_forecasts - naive_forecast)
        while np.quantile(plain_kernel_X_naive_div, q_data_div) == 0 or np.quantile(plain_kernel_X_naive_div_fore, q_data_div) == 0:
            q_data_div += 0.01
        width_div = laplace_quantile_div/np.quantile(plain_kernel_X_naive_div, q_data_div)
        intermediate_div_kernel = np.exp(-np.abs(width_div) * plain_kernel_X_naive_div)
        div_kernel = 1 - div_impact*intermediate_div_kernel
        nominator = np.exp(-np.abs(width) * plain_kernel_X - 1/(2*sigma**2) * plain_kernel_X_naive)
        div_kernel[~((nominator != 1) & (intermediate_div_kernel < (1/div_impact)*(1 - nominator)))] = nominator[~((nominator != 1) & (intermediate_div_kernel < (1/div_impact)*(1 - nominator)))]
        training_matrix_cor_naive_div = nominator/div_kernel

        reg_corr_naive = SVR(kernel = 'precomputed', epsilon = epsilon , C = C)
        kernel_alignment = kernel_target_alignment(training_matrix_cor_naive_div, Y_train)
    ...
    return forecast, np.mean(np.abs(insample_fit - Y_train)), kernel_alignment, insample_fit
