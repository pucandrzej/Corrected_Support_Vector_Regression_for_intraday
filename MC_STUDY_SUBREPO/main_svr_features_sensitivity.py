...
def kernel_target_alignment(K, y):
    ...
    denominator = np.sum(y**2) * np.linalg.norm(K, ord='fro') # TODO confirm that this was an error (sqrt on y) and fix everywhere else in case it was indeed
...
def generalized_pf(coeffs, ranking):
    ...
    for i in range(len(coeffs)):
        w_0 = np.sum(coeffs[:i+1] * D[i, :i+1]) # important: the shift by 1 to account for python indexing and make it <=
        w_1 = []
...
def mc_step(params):
    ...
    cov_1 = np.eye(int(n_features / 3)) * 1  # Covariance matrix for the first distribution, 3 is the variance TODO vary the correlation coeffs
    cov_1[cov_1 == 0] = corr_config[0]
    cov_2 = np.eye(int(n_features / 3)) * 1  # Covariance matrix for the second distribution
    cov_2[cov_2 == 0] = corr_config[1]
    cov_3 = np.eye(int(n_features / 3)) * 1  # Covariance matrix for the third distribution
    ...
    Y = generate_ARX_multiexog([alpha], exog_coeff, 1, X_features, sample_len, step_seed)
    ...
    X = np.vstack((
        Y[:-1],
        *[exog_var[1:] for exog_var in exog_standardised_vars]
    )).T
    Y_train = Y[1:]
    ...
    for label in np.unique(clustering.labels_):
        ...
        cluster_mean_coeffs.append(np.mean(np.abs(exog_coeff[clustering.labels_ == label])))
        ...
        clusters_replaced_by_one_variable.append(np.hstack([X[:, 1:][:, clustering.labels_ != label], X_cluster[:, np.argmax(inter_kernel_alignments), None]]))
        clusters_replaced_by_avg_variable.append(np.hstack([X[:, 1:][:, clustering.labels_ != label], np.mean(X_cluster, axis=1)[:, None]]))
        all_clusters_replaced_by_vars.append(X_cluster[:, np.argmax(inter_kernel_alignments)])
    ...
    for label_idx, label in enumerate(np.unique(clustering.labels_)):
        two_clusters_replaced_by_vars.append(np.hstack([X[:, 1:][:, clustering.labels_ == label], np.delete(all_clusters_replaced_by_vars, label_idx, axis=1)]))
    
    all_clusters_replaced_by_avgs = np.vstack(all_clusters_replaced_by_avgs).T
    # prepare replacements of 2 clusters by variables
    for label_idx, label in enumerate(np.unique(clustering.labels_)):
        two_clusters_replaced_by_avgs.append(np.hstack([X[:, 1:][:, clustering.labels_ == label], np.delete(all_clusters_replaced_by_avgs, label_idx, axis=1)]))

    subsets_maes = []
    subsets_aligns = []
    subsets_max_aligns = []

    for subset_idx, variables_subset in enumerate([X[:, 1:]] + clusters_replaced_by_one_variable.copy() + two_clusters_replaced_by_vars.copy() + [all_clusters_replaced_by_vars.copy()] + clusters_replaced_by_avg_variable.copy() + two_clusters_replaced_by_avgs.copy() + [all_clusters_replaced_by_avgs.copy()]):
        variables_subset = np.hstack([variables_subset, X[:, 0, None]]) # add the AR part back
        ...
        for q_kernel in prod_mc_config['param_grid_slide_on_median']['q_kernel']:
            for q_data in prod_mc_config['param_grid_slide_on_median']['q_data']:
                for C in prod_mc_config['param_grid_slide_on_median']['C']:
                    for epsilon in prod_mc_config['param_grid_slide_on_median']['epsilon']:
                        intermediate_scores = []
                        intermediate_aligns = []
                        for train_idx, test_idx in tscv.split(variables_subset):
                            X_train_cv, X_test_cv = variables_subset[train_idx], variables_subset[test_idx]
                            ...
                            kernel_alignment = kernel_target_alignment(training_matrix, y_train_cv)
                            reg.fit(training_matrix, y_train_cv)

                            plain_kernel_X = np.sqrt(np.sum((X_train_cv - X_test_cv)**2, axis=1))
                            width = np.log(2 - 2*q_kernel)/np.quantile(plain_kernel_X, q_data)
    ...
    corr_generalized_spearmans_footrol = generalized_pf(cor_matrix[:, 1], cor_matrix[:, 0])

    align_matrix = np.vstack([kernel_alignments, np.abs(np.array([alpha] + list(exog_coeff)))]).T
    ...
    align_generalized_spearmans_footrol = generalized_pf(align_matrix[:, 1], align_matrix[:, 0])

    dropping_results = pd.DataFrame()
    dropping_results["Meta"] = ['_', '0', '1', '2', '12', '02', '01', '012', '0 avg', '1 avg', '2 avg', '12 avg', '02 avg', '01 avg', '012 avg']
    ...
    return {'corr_generalized_spearmans_footrol': corr_generalized_spearmans_footrol, 
    ...
    for corr_config in correlations:
        ...
        for alpha in [0.1, 0.9]:
            ...
            for sample_length in sample_lengths:
                params = [(corr_config, alpha, sample_length, seeds[idx], idx) for idx in range(mc_steps)]
                ...
                pickle.dump(results, open(os.path.join('MC_RESULTS', dir_name, current_datetime + '_results.pickle'), 'wb'))
