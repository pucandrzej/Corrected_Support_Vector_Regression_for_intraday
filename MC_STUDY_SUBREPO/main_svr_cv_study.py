...
from processes_generators.generate_sample_and_predictors import generate_sample_and_predictors
...
parser.add_argument('--mode', default='dev', help='Specifies whether to use the devel or prod confid files.')
parser.add_argument('--cv_run', default=False, action='store_true', help='To use CV values if True or hand picked when False.')
parser.add_argument('--cv_recalc', default=False, action='store_true', help='To recalculate CV values if True or oad from CV cache when False.')
parser.add_argument('--use_base_forecast_corr', default=False, action='store_true', help='Use the base model forecasts for kernel correction.')
...
def mc_step(params):
    ...
    step_results_df[f'{model}_MAE'] = [np.mean(np.abs(np.array(Y_fore) - np.array(base_model_fores)))]
    step_results_df[f'SVR_MAE'] = [np.mean(np.abs(np.array(Y_fore) - np.array(svr_fores)))]
    step_results_df[f'cSVR_MAE'] = [np.mean(np.abs(np.array(Y_fore) - np.array(csvr_fores)))]
    step_results_df[f'ccSVR_MAE'] = [np.mean(np.abs(np.array(Y_fore) - np.array(ccsvr_fores)))]
...
if __name__ == '__main__':
    ...
    for model in mc_config['models']:
        for sample_length in sample_lengths:
            for params_index, models_params in enumerate(mc_config['model_params_sets'][model]):

                dir_name = f"{model}_params_set_{params_index}_sample_length_{sample_length}"
                ...
                for mc_step_cv in range(mc_steps):
                    X, Y = generate_sample_and_predictors(model, models_params, sample_length, seeds[mc_step_cv])
                    i = sample_len//2 - 1
                    X_train = X[:i, :]
                    Y_train = Y[1:i+1]
                    ...
                    kernel_model = 'SVR'
                    SVR_parameters = prepare_params_combinations(kernel_model, mc_config['param_grid_slide_on_median'])
                    params_cv = [(kernel_model, model, X_train, Y_train, tscv, args.use_base_forecast_corr, Y[:i], param) for param in SVR_parameters]
                    with Pool(processes=min(30, len(SVR_parameters))) as p:
                        results_svr = p.map(parallel_cv, params_cv.copy())
                    # get the best params
                    best_params[kernel_model].append(SVR_parameters[np.argmin(results_svr)])
                    all_mae_cv[kernel_model].append(results_svr)

                    # use the SVR best models to also search for the cSVR best models
                    kernel_model = 'cSVR'
                    cSVR_parameters = prepare_params_combinations(kernel_model, mc_config['param_grid_slide_on_median'], best_params_svr=best_params['SVR'][-1])
                    params_cv = [(kernel_model, model, X_train, Y_train, tscv, args.use_base_forecast_corr, Y[:i], param) for param in cSVR_parameters]
                    with Pool(processes=min(30, len(cSVR_parameters))) as p:
                        results_csvr = p.map(parallel_cv, params_cv.copy())
                    # get the best params
                    best_params[kernel_model].append(cSVR_parameters[np.argmin(results_csvr)])
                    all_mae_cv[kernel_model].append(results_csvr)

                    # use the cSVR best models to also search for the ccSVR best models
                    kernel_model = 'ccSVR'
                    ccSVR_parameters = prepare_params_combinations(kernel_model, mc_config['param_grid_slide_on_median'], best_params_svr=best_params['cSVR'][-1])
                    params_cv = [(kernel_model, model, X_train, Y_train, tscv, args.use_base_forecast_corr, Y[:i], param) for param in ccSVR_parameters]
                    with Pool(processes=min(30, len(ccSVR_parameters))) as p:
                        results_ccsvr = p.map(parallel_cv, params_cv.copy())
                    # get the best params
                    best_params[kernel_model].append(ccSVR_parameters[np.argmin(results_ccsvr)])
                    all_mae_cv[kernel_model].append(results_ccsvr)

                # plot the cv results and save them
                plot_cv_results({'SVR': SVR_parameters.copy(), 'cSVR': cSVR_parameters.copy(), 'ccSVR': ccSVR_parameters.copy()}, all_mae_cv.copy(), model, sample_length, params_index)

                params = [(model, models_params, sample_length, seeds[idx], best_params, idx) for idx in range(mc_steps)]
                ...
                pickle.dump({'SVR': SVR_parameters, 'cSVR': cSVR_parameters, 'ccSVR': ccSVR_parameters}, open(os.path.join('MC_RESULTS', dir_name, current_datetime + '_CV_parameters.pickle'), 'wb'))
                pickle.dump(all_mae_cv, open(os.path.join('MC_RESULTS', dir_name, current_datetime + '_CV_mae.pickle'), 'wb'))
                pd.DataFrame([pd.concat(results).mean(axis=0)]).to_csv(os.path.join('MC_RESULTS', dir_name, current_datetime + '.csv'))
